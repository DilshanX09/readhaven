...
# Query to fetch books with necessary joins ==> tables list [books , book_category , author_name , book_img]
$query = "SELECT * FROM books 
    ...
    INNER JOIN book_img ON books.book_id = book_img.book_id";
...
/* 
     * This block only execute if min and max are not 0 
     * hint -> min and max is already choose 
*/
if ($min != 0 && $max != 0) {
     $where[] = "books.price BETWEEN '$min' AND '$max'";
} else if ($min != 0) {
     $where[] = "books.price >= '$min'";
} else if ($max != 0) {
     $where[] = "books.price <= '$max'";
}
/* * this block only execute if keyword are not empty */
if (!empty($keyword)) {
...
/* $where array is not empty && executed blockes pushed querys bind using AND ..... AND ..... */
if (count($where) > 0) {
     $query .= " WHERE " . implode(" AND ", $where);
}

/*
     ** Check current $where[] include querys 
